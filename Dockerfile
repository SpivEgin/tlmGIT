...
RUN mkdir /opt/tlm/
ADD files/gogs_0.11.43_linux_amd64.zip /opt/tlm/
ADD files/usersList.txt /opt/tlm
ADD bash/main.sh /opt/tlm/
RUN apt-get -y update && apt-get -y install unzip git &&\
    cd /opt/tlm/ && unzip gogs_0.11.43_linux_amd64.zip && mkdir /opt/tlm/gogs/custom &&\
    chmod +x /opt/tlm/main.sh &&\
    apt-get -y autoclean &&\
    rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/* /opt/tlm/gogs_0.11.43_linux_amd64.zip
